...
public class BlogServiceImpl implements BlogService {
    ...
    public Mono<Blog> save(Blog blog) {
        ...
        return blogRepository.findByAuthorId(blog.getAuthorId()).count().flatMap(size -> {
            return !(size>2) ? this.blogRepository.save(blog): Mono.error(new BlogLimitException("Ha superado la cantidad limite de Blog por Author"));
        });
